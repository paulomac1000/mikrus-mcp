...
def test_durable_remote_job_real_disconnect_reconciliation() -> None:
    ...
    async def scenario() -> None:
        ...
        reopened: InvocationKernel | None = None
        try:
            ...
            if job_id is not None:
                try:
                    cleanup_target = reopened if reopened is not None else kernel
                    cleanup_identity = cleanup_target.registry.resolved_identity("real")
                    cleanup_arguments = {"job_id": job_id, "reason": "cleanup"}
                    approvals.issue_for_test(
                        "remote_job_cancel",
                        "posix-uid-real",
                        cleanup_identity,
                        job_id,
                        normalized_arguments_digest(cleanup_arguments),
                    )
                    await cleanup_target.invoke("remote_job_cancel", cleanup_arguments, caller)
                except Exception:  # noqa: S110
                    pass
            try:
                await kernel.registry.close()
            except Exception:  # noqa: S110
                pass
