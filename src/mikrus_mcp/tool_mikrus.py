...
async def describe_mikrus_capabilities(ctx: Context[AppContext]) -> ToolResult:
...
async def list_configured_servers(ctx: Context[AppContext]) -> ToolResult:
...
async def get_server_info(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
    """Return basic information for one exact mikr.us target."""
    return await _invoke(ctx, "get_server_info", {"server": server})


async def list_servers(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
    """List VPS resources for the account bound to one exact mikr.us target."""
    return await _invoke(ctx, "list_servers", {"server": server})


async def get_server_stats(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
...
async def get_logs(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
...
async def get_db_info(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
...
async def get_ports(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
    """Return assigned TCP and UDP ports for one exact mikr.us target."""
    return await _invoke(ctx, "get_ports", {"server": server})


async def get_cloud(ctx: Context[AppContext], server: str | None = None) -> ToolResult:
