...
- `src/mikrus_mcp/config.py` — loads configuration from environment variables (`.env`). Supports legacy single-server, multi-server JSON, and SSH-only modes.
...
| `MIKRUS_API_KEY` | Yes* | API key from the mikr.us panel (legacy mode) |
| `MIKRUS_SERVER_NAME` | Yes* | Server identifier (legacy mode) |
